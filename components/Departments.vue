<template>
    <div style="background-color: #F1F3F4" class="pt-8 pb-15">
      <v-container>
        <h1 class="text-center">{{fr_dict.our_departments}}</h1>
        <p class="text-center" style="font-size: 12px;">{{fr_dict.our_departments_description}}</p>
        <br>
        <v-row justify="center">
          <v-col cols="12" sm="6" md="4" lg="3" v-for="(department, i) in departments" :key="i">
            <v-card class="mx-auto" max-width="300" elevation="5" outlined data-aos="fade-up" :data-aos-delay="100 * i">
              <v-img height="150" :src="'https://site.ens-bechar.dz/ensbechar/storage/app/public/' + department.src"></v-img>
              <v-col>
                ...
                <v-row class="pt-3">
                  <v-col cols="2">
                    <v-avatar size="36px">
                      <v-img height="36" width="36" :src="'https://site.ens-bechar.dz/ensbechar/storage/app/public/' + department.rs_image"></v-img>
                    </v-avatar>
                  </v-col>
                  <v-col cols="10">
                    <div class="user-info">
...
<script>
  ...
  export default {
      ...
      data() {
        return {
          fr_dict: {
            our_departments: "Notre départements",
